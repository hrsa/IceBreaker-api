...
export class TranslationService {
  ...
  async fillMissingTranslations(
    ...
    const cardsWithMissingTranslations = allCards.filter(card => {
      const hasAtLeastOneLanguage = allLanguages.some(lang =>
        this.languageUtilsService.hasPropertyByLanguage(card, 'question', lang)
      );
  ...
  async fillMissingCategoryTranslations(
    ...
    const categoriesWithMissingTranslations = allCategories.filter(category => {
      const hasAtLeastOneNameLanguage = allLanguages.some(lang =>
        this.languageUtilsService.hasPropertyByLanguage(category, 'name', lang)
      );
      const hasMissingNameLanguages = allLanguages.some(lang =>
        !this.languageUtilsService.hasPropertyByLanguage(category, 'name', lang)
      );
      ...
      const hasMissingDescLanguages = allLanguages.some(lang =>
        !this.languageUtilsService.hasPropertyByLanguage(category, 'description', lang)
      );

      return (hasAtLeastOneNameLanguage && hasMissingNameLanguages) || 
             (hasAtLeastOneDescLanguage && hasMissingDescLanguages);
    });

    const categoriesToProcess = categoriesWithMissingTranslations.slice(0, limit);

    this.logger.log(`Found ${categoriesWithMissingTranslations.length} categories with missing translations. Processing ${categoriesToProcess.length}.`);

    const updatedCategories: Category[] = [];

    for (const category of categoriesToProcess) {
      await this.translateCategoryProperty(
        category, 
        'name', 
        sourceLanguage, 
        targetLanguages, 
  ...
  private async translateCategoryProperty(
    ...
    const availableLanguages = allLanguages.filter(lang =>
      this.languageUtilsService.hasPropertyByLanguage(category, propertyPrefix, lang)
    );
