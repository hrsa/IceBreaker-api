...
export class CategorySelectionState implements BotState {
  ...
  async handle(ctx: Context): Promise<void> {
    ...
    await this.telegramService.updateOrSendMessage(
      ...
      this.telegramService.createCategoryKeyboard(categories, ctx.session.language, ctx.session.selectedCategoryIds || [])
    );
  }
  ...
  async next(ctx: Context): Promise<void> {
    if (!ctx.match) {
      await this.telegramService.updateOrSendMessage(
        ctx,
        this.translate.t("telegram.category.selection.invalid", { lang: ctx.session.language })
      );
      return;
    }
    ...
    const selectedText = this.translate.t("telegram.category.selection.selected_count", {
      args: { count: ctx.session.selectedCategoryIds.length },
  ...
  async complete(ctx: Context): Promise<void> {
    if (!ctx.session.selectedCategoryIds || !ctx.session.selectedCategoryIds.length) {
      await this.telegramService.updateOrSendMessage(
        ctx,
        this.translate.t("telegram.category.selection.select_at_least_one", { lang: ctx.session.language })
      );
      return;
    }
